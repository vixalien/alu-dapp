...
// At least 3 sample holders — replace with real Sepolia addresses after deploy
...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (address) {
      const [bal, pct] = await Promise.all([token.balanceOf(address), token.ownershipPercentage(address)]);
      setMyBalance(formatEther(bal as bigint));
      setMyPct((pct as bigint).toString());
    }

    const holderData = await Promise.all(
      SAMPLE_HOLDERS.map(async (addr) => {
        const [bal, pct] = await Promise.all([token.balanceOf(addr), token.ownershipPercentage(addr)]);
        return { address: addr, balance: parseFloat(formatEther(bal as bigint)).toLocaleString(), pct: (pct as bigint).toString() };
      })
    );
  ...
  async function handleDistribute(e: React.FormEvent) {
    e.preventDefault();
    if (!signer) return;
    setDistError(null); setDistSuccess(null);
    if (!isAddress(recipient)) { setDistError("Invalid wallet address."); return; }
    const wei = parseEther(amount);
    if (wei <= 0n) { setDistError("Amount must be greater than zero."); return; }
    ...
    try {
      ...
      setDistError((err as { reason?: string })?.reason ?? (err instanceof Error ? err.message : "Transaction failed"));
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Supply", value: `${parseFloat(totalSupply || "0").toLocaleString()} ALUT` },
          { label: "Your Balance", value: `${parseFloat(myBalance || "0").toLocaleString()} ALUT` },
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {holders.map((h) => (
              <tr key={h.address}>
                <td className="px-6 py-3 font-mono text-gray-700">{h.address.slice(0,6)}...{h.address.slice(-4)}</td>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <form onSubmit={handleDistribute} className="space-y-4">
            ...
            <div>
              ...
              <input type="number" placeholder="e.g. 100000" value={amount} min="1"
