    ...
    <div className="max-w-2xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          {preview && (
            <img
              src={preview}
              alt="Preview"
              className="max-h-40 rounded-lg mx-auto bg-gray-300 overflow-clip"
            />
          )}
          {computedHash && (
      ...
      {tab === "hash" && (
        <div className="space-y-4">
          ...
          />
          <button
            onClick={() => verify(pastedHash)}
            disabled={!isValidBytes32(pastedHash) || isVerifying}
            className="w-full py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50"
          >
        ...
        <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-6">
          ...
          <dl className="text-sm text-green-700 space-y-1">
            ...
            <div>
              ...
              <dd className="inline">{result.date}</dd>
            </div>
            <div>
              <dt className="inline font-medium">By: </dt>
              <dd className="inline font-mono">
                {result.registrant.slice(0, 6)}...{result.registrant.slice(-4)}
              </dd>
            </div>
          </dl>
        </div>
