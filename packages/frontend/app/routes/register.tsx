...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    e.preventDefault();
    if (!signer) return;
    setIsRegistering(true); setError(null); setResult(null);
    try {
      const registry = getRegistryWithSigner(signer);
      const tx = await registry.registerAsset(name, fileType, hash);
      const receipt = await tx.wait();
      const event = receipt.logs
        .map((log: unknown) => { try { return registry.interface.parseLog(log as { topics: string[]; data: string }); } catch { return null; } })
        .find((e: unknown) => (e as { name?: string } | null)?.name === "AssetRegistered");
      setResult({ tokenId: event?.args?.tokenId?.toString() ?? "1", txHash: receipt.hash });
    } catch (err: unknown) {
      const msg = (err as { reason?: string })?.reason ?? (err instanceof Error ? err.message : "Transaction failed");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        {preview && <img src={preview} alt="Preview" className="max-h-40 mt-4 rounded mx-auto" />}
        {hash && (
      ...
      {/* Step 2 */}
      <form onSubmit={handleRegister} className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
        <h2 className="font-semibold text-gray-900">Step 2 — Register on Blockchain</h2>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Asset Name</label>
          <input type="text" placeholder="e.g. ALU Official Logo 2026" value={name}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Content Hash (bytes32)</label>
          <input type="text" value={hash} readOnly placeholder="Upload a file above to generate hash"
            className="w-full border border-gray-200 rounded-lg px-4 py-2 text-sm font-mono bg-gray-50 text-gray-500" />
        </div>
        <button type="submit" disabled={!hash || !name || isRegistering}
